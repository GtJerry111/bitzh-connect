...
**主窗口改造（§4）整体撤回**，菜单栏面板设计（§3）与架构切换（§2）保留。

原因（真机验证结论）：液态玻璃的质感依赖「背后是壁纸」这一前提——
菜单栏面板成立（弹出时背后即壁纸），常驻主窗口不成立（背后多为其他窗口，
浅色糊成近白、深色糊成近黑）；且卡片容器会遮蔽校训水印（核心品牌时刻）。

最终方案 = **方案 A（回滚）**：
- 主窗口恢复原排版（hero 自由呼吸 + 校训水印 + 裸表单），不装玻璃、不做卡片；
- 仅保留断开按钮「白底绿描边」双态样式（与本次重设计正交的改进）；
