...
- **TUN 全局路由默认开启**：所有流量（含 SSH 等裸 TCP）都走 VPN；主界面一键切换代理/TUN 模式；首次连接安装一次特权服务，之后每次连接免密
- **实时仪表盘**：连接时长、上行/下行速率 + 60 秒实时波形图；TUN 模式按虚拟网卡计数，macOS 代理模式按进程采样内核流量，两种模式下速率都真实可见
- **校内导航**：10 个常用站点（珠海校区/校本部分组），折叠收纳不占地方
- **断线自动重连**：退避重试 + 系统休眠唤醒后自动恢复；认证失败/手动断开不重试，连接稳定存活后重置重试计数；内置连接看门狗，监测服务器路由被抢与内核假死并自动恢复
- **BIT 品牌视觉**：校徽绿主题、校训书法水印、深浅色三态（跟随系统/浅色/深色）
- 与 **EasyConnect** 相比更快速、更轻量
- 基于 PySide6，易于构建，方便初学者参与维护
- 跨平台支持，对 **macOS** 版本进行了原生适配和优化
- macOS 原生体验：菜单栏常驻图标，左键展开液态玻璃快捷面板（macOS 26+，旧系统毛玻璃；连接开关/速率/模式/校内导航），右键快捷菜单可开启「在菜单栏显示速度」——图标旁实时显示两行速率，随菜单栏深浅色自动反色
- 密码本地加密保存，不明文落盘，无需访问系统钥匙串
- 连接诊断日志本地落盘（自动轮转、不含密码），高级设置中一键打开日志目录，断线后可事后定位原因
...
> 3. TUN 模式下仪表盘按虚拟网卡计数显示真实速率；macOS 下代理模式也会按进程采样内核流量显示速率（其他平台代理模式速率显示为 "—"）
...
| 菜单栏快捷面板（液态玻璃） | 菜单栏速率显示（右键菜单开启） |
| --- | --- |
| <img width="412" alt="菜单栏快捷面板" src="assets/screenshot-menubar.png" /> | <img alt="菜单栏速率显示" src="assets/screenshot-menubar-speed.png" /> |
