...
- **TUN 全局路由默认开启**：所有流量（含 SSH 等裸 TCP）都走 VPN；主界面一键切换代理/TUN 模式
- **实时仪表盘**：连接时长、上行/下行速率 + 60 秒实时波形图，连接状态一目了然
- **校内导航**：10 个常用站点（珠海校区/校本部分组），折叠收纳不占地方
- **断线自动重连**：退避重试 + 系统休眠唤醒后自动恢复连接
- **BIT 品牌视觉**：校徽绿主题、校训书法水印、深浅色三态（跟随系统/浅色/深色）
- 与 **EasyConnect** 相比更快速、更轻量
- 基于 PySide6，易于构建，方便初学者参与维护
- 跨平台支持，对 **macOS** 版本进行了原生适配和优化
- macOS 原生体验：菜单栏常驻图标，左键展开液态玻璃快捷面板（macOS 26+，旧系统毛玻璃；连接开关/速率/模式/校内导航），右键快捷菜单
